...
class UserTest < ActiveSupport::TestCase
  ...
  test 'filtered search' do
    ...
    users = User.filtered_search('magick')
  
    assert_equal 9, users.count
    assert users.all? { |u| u.to_s =~ /magick/ }
  
    users = User.filtered_search('magick_name')
  
    assert_equal 6, users.count
    assert users.all? { |u| u.to_s =~ /magick_name/ }
  
    users = User.filtered_search('magick_name magick_lastname')
  
    assert_equal 1, users.count
    assert users.all? { |u| u.to_s =~ /magick_name magick_lastname/ }
  
    users = User.filtered_search(
      "magick_name #{I18n.t('filtered_search.or').first} magick_lastname"
    )
  
    assert_equal 9, users.count
    assert users.all? { |u| u.to_s =~ /magick_name|magick_lastname/ }
  
    users = User.filtered_search('nobody')
