...
class GroupTest < ActiveSupport::TestCase
  ...
  test 'filtered search' do
    ...
    groups = Group.filtered_search('magick')
  
    assert_equal 6, groups.count
    assert groups.all? { |u| u.to_s =~ /magick/ }
  
    groups = Group.filtered_search('magick_name')
  
    assert_equal 5, groups.count
    assert groups.all? { |u| u.to_s =~ /magick_name/ }
  
    groups = Group.filtered_search(
      "magick_something #{I18n.t('filtered_search.or').first} magick_name"
    )

    assert_equal 6, groups.count
    assert groups.all? { |u| u.to_s =~ /magick_name|magick_something/ }
  
    groups = Group.filtered_search('nobody')
